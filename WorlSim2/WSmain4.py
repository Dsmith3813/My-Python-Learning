...
world = []              # File of the condition of the world per year
avgDR = []  # after a given cycle (epoch)
avgBR = []

# In memory accumulators and flags
Population = 0          # program Population
year = 0                # In memory variable
EvePassed = 0           # Flags
AdamPassed = 0
FoodStore = pm.NaturalPlanetFood  # In memory food storage container.

# WSParm? entries that change over time is stored here and modified here
# by the program
...
def init():
    ...
    residing in the world and stores the information in the Peeps table. 
    Initial settings for Adam and Eve are stored in the parm file 
    WSparms?.py
    '''
    global Population

    # Setup Adam  See WSpeeps?.py for details
    ...
    # Set up Eve See WSpeeps?.py for details
...
def FieldOfDreams():
    ...
    for key, attrib in Peeps.items():
        if attrib[pm.Age] >= pm.WorkAge and attrib[pm.Age] < WorkAgeEnd:
            AblePeeps += 1
    ...
    if year > 50 and t.flip(pm.famine):
        FoodGen -= int(FoodGen * pm.Spoilage)
        FamineCount += 1
...
def lifeIsGood(LGyear):
    ...
    global FoodStore, life, longlife, WorkAgeEnd, EndBirth, StartBirth, BirthMiddleAge, Population, avgBR, avgDR
